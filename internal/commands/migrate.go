...
var scheme = []Scheme{
	...
	},
	{
		...
		Query: `
				INSERT INTO users(user_id, role, password)
                           SELECT 'Admin01', 'EMPLOYEE', '$2a$10$NKtnMwDPFSQLG6uOi4Zqheru5Ygbj9TWFHjpl478rRSaO5cJ9QuH2'
                          WHERE NOT EXISTS (SELECT user_id FROM users WHERE user_id = 'Admin01');
			`,
	},
...
func MigrateUP(db *postgresql.Database) {
	...
	for _, s := range scheme {
		if s.Index > version {
			...
			if _, err = db.Exec(fmt.Sprintf(`User ID UPDATE schema_migrations SET version = %d`, s.Index)); err != nil {
				log.Fatalln("migrate error", err)
			}
		}
	}
